...
class ModelEvaluator:
    ...
    def load_model(self):
        ...
        if self.config.output_screen:
            self.pred_fn = jax.jit(lambda p, b: model.predict(p, b,
                                                              output_screen=True))
        else:
            self.pred_fn = jax.jit(model.predict)
    ...
    def prepare_data(self):
        ...
        self.source_valid = source_valid  # Store for later use
    ...
    def evaluate(self):
        ...
        for i in range(nr_dats):
            ...
            print(f'{atoms.arrays["charges"].shape=} {atoms.arrays["screen"].shape=}')
