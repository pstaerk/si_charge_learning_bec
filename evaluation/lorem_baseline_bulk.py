...
pred_fn, apply_fn, species_to_weight, params, cutoff = load_model(
    '../training_runs/lorem_baseline_alldata/run/checkpoints/R2_E+F/',
...
def extract_results(results, batch, labels, params):
    ...
    for key in ['energy', 'forces', 'apt', 'charges']:
        if key == "energy":
            ...
            if key in results:
                charges = results[key][...,0]
                actual_results[key] = np.array(charges[batch.node_mask])
        elif key == 'apt':
            if key in results:
...
output_path = Path('/work/pstaerk/outs/lorem_baseline_alldata_bulk/')
